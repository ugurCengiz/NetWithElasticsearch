...
namespace NetWithElasticsearch.Controllers
{
    ...
    public sealed class ValuesController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateData(CancellationToken cancellationToken)
        {
            ...
            {
                var title = new string(Enumerable.Repeat("abcdefgğhıijklmnoöprsştuwyz", 5).Select(s => s[random.Next(s.Length)]).ToArray());

                var words = new List<string>();
                for (int j = 0; j < 500; j++)
                {
                    words.Add(new string(Enumerable.Repeat("abcdefgğhıijklmnoöprsştuwyz", 5).Select(s => s[random.Next(s.Length)]).ToArray()));
                }
        ...
        public async Task<IActionResult> GetDataListWithEF(string value)
        {
            IList<Travel> travels =
                await context.Set<Travel>().Where(x => x.Description.Contains(value))
                    .AsNoTracking()
                    .ToListAsync();
